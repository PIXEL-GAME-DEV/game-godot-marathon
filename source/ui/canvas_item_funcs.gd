...
static func get_aa(canvas_item: CanvasItem, size := 1.0) -> float:
	if not canvas_item.is_inside_tree():
		canvas_item.draw_set_transform(Vector2.ZERO, 0, Vector2.ONE)
		return 1.0
	...
	if viewport:
		scale_factor = viewport.get_oversampling()
